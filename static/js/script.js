...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const mediumLeaderboard = document.getElementById("medium-leaderboard");
    ...
    function saveToLeaderboard() {
      ...
      if (
        leaderboard[difficulty].length < 10 ||
        strikes < leaderboard[difficulty][9].strikes ||
        (strikes === leaderboard[difficulty][9].strikes &&
          finalTime.localeCompare(leaderboard[difficulty][9].time) < 0)
      ) {
    ...
    submitLeaderboardBtn.addEventListener("click", () => {
      ...
      leaderboard[difficulty].push({ name: playerName, time: finalTime, strikes: strikes });
