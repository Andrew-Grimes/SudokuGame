...
# Ensure PyScript is only run in the browser
try:
...
# Run only if inside the browser (PyScript)
if browser_env:
    ...
    def create_board(board, solved_board):
        """Generates the Sudoku board dynamically"""
        global solution
        ...
        for i in range(9):
            for j in range(9):
                ...
                if board[i][j]["prefilled"]:
                    ...
                    input_element.addEventListener("click", lambda event, num=input_element.value: highlight_numbers(num))
                else:
                    input_element.addEventListener("input", handle_input)

                # Apply border styles for 3x3 separation
                if i % 3 == 0 and i != 0:
    ...
    def start_game(event):
        """Fetches a new Sudoku game based on difficulty"""
        global strikes
        strikes = 0
    ...
    def handle_input(event):
        """Handles user input & strike system"""
        global strikes
        ...
        if value == solution[row][col]:
            ...
            input_element.disabled = True
            input_element.addEventListener("click", lambda event, num=value: highlight_numbers(num))
        else:
            input_element.value = ""
            strikes += 1
            strike_counter_element.innerText = f"Strikes: {strikes}"
